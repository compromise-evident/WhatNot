/// Minimalist byte occurrence print-out for any file. Insert name on line 22.
...
Depending on byte endings and encoding,  some characters may show up here as two
separate items--for which two separate occurrence counters tick once. This issue
does not exist if you're strictly dealing with files created using C++. Enjoy*/
...
{	ifstream in_stream;
	...
	in_stream.open("my file");
	if(in_stream.fail() == true) {in_stream.close(); cout << "No file/bad dir."; return 0;}
	in_stream.get(temp_file_byte);
	if(in_stream.eof() == true)  {in_stream.close(); cout << "File empty.";      return 0;}
	...
	for(int a = 0; a < 256; a++)
	{	     if(a <  10) {cout << "occurence of   " << a << ":  ";}
		else if(a < 100) {cout << "occurence of  "  << a << ":  ";}
		else             {cout << "occurence of "   << a << ":  ";}
...
Dec	Hex	 Binary      HTML   Char  Description                  C++ file read/write
 ...
 13  0D  00001101    &#13;  CR    Carriage Return              out_stream.put(13);
